...
reducedplot <- function(model,size)
{
  ...
  for(size in c(80,400))
  {
    for(j in j.index)
    {
      for(k in k.index)
      {
        ...
        tmp <- result.stat%>% mutate ("size" = size, "LB" = LB.glob, "MAST" = MAST.glob, "KW-II" = Wg.glob,"i" = i,"j" = j,"k" = k,"batch" = as.character(result$setting$kappa[4]),"effect" = as.character(result$setting$delta[4]))%>%dplyr::select("LB","LN","MAST","KW","KW-II","DESeq2","MGS","i","j","k","batch","effect","size")
  ...
  res3 %>%
    ...
    ylab("rejection rate") + ggtitle(paste0("type-I error"))+
  ...
  ggsave(file = paste0("../",model,"_null_size.png"), p, width = 16, height=12)
  p
}
