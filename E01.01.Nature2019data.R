...
DRNA = "RNA" # or DNA
  ...
  library(dplyr)
  nm = switch(DRNA, 
              DNA = "Nature2019data/MGX_genefamilies.tsv",  # currently not available.
              RNA = "Nature2019data/MTX_genefamilies.tsv") %>% print
  ...
  names(b) = gsub("_.*$", "", names(b))  # "CSM5FZ4M_Abundance.RPKs" => "CSM5FZ4M"
  ...
  b.meta <- 
    ...
    dim(b.meta) # 104 subjects x 10 variables (for RNA)
                # 109 subjects x 10 variables (for DNA)
    
    # rearrangement of the RNA data
    sample.list = b.meta$External.ID
    sample.index.in.b = sapply(sample.list, function(x) which(x == names(b))[1])

### 1. full data (gene total and gene-species)
  dat = 
    list(otu = b[, sample.index.in.b],
         meta = b.meta,
         taxa = data.frame(gene.bact = b$X..Gene.Family, gene = NA, bact = NA))
...
### 2. Separating out the joint and the marginal data
  ...
  saveRDS(dat.joint, sprintf("Nature2019data/data.geneRPK.joint.%s.rds", DRNA))
  saveRDS(dat, sprintf("Nature2019data/data.geneRPK.full.%s.rds", DRNA))
  saveRDS(dat.marginal, sprintf("Nature2019data/data.geneRPK.marginal.%s.rds", DRNA))

  dat.joint$taxa$gene.bact %>% length # 1,247,575
  dat.joint$taxa$gene %>% unique %>% length # 917,164
  dat.joint$taxa$bact %>% unique %>% length # 360
...
### 3. marginal bacteria data
  ...
  saveRDS(dat.bact, sprintf("Nature2019data/data.bactRPK.marginal.%s.rds", DRNA))
