summary.gamlss2 <-  # suppresses printings and only extracts mu coef and pvals.
  function (object, type = c("vcov", "qr"), robust = FALSE, save = FALSE, 
          hessian.fun = c("R", "PB"), digits = max(3, getOption("digits") - 3), ...) {
    type <- match.arg(type)
    if (type == "vcov") {
      covmat <- try(suppressWarnings(vcov(object, type = "all", 
                                          robust = robust, hessian.fun = hessian.fun)), silent = TRUE)
    ...
    if (type == "vcov") {
      ...
      #   if (length(eval(parse(text = paste(paste("object$", 
      #                                            "mu", sep = ""), ".fix==TRUE", sep = "")))) != 
      #       0) {
      #     pm <- 1
      #     p1 <- 1:pm
    ...
    if (type == "qr") {
      estimatesgamlss <- function(object, Qr, p1, coef.p, est.disp, 
                                  df.r, digits = max(3, getOption("digits") - 3), covmat.unscaled, 
                                  ...) {
