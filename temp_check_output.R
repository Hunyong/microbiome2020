...
list.files(".", pattern = "tmp_[0-9]+") %>% 
  ...
  transmute(i = gsub(ptrn, "\\1", a) %>% as.numeric,
            ...
            s = gsub(ptrn, "\\7", a) %>% as.numeric) %T>% 
  print %>% 
  write.csv("tmp_bookkeeping.csv")


ptrn = ".*stat-n([0-9]*)-pert(.*)-([a-z]*)-([0-9]*)\\.([0-9]*)\\.([0-9]*)\\.rds"
list.files("output", pattern = "stat.*\\.rds") %>% 
  data.frame(a = .) %>% 
  transmute(i = gsub(ptrn, "\\4", a) %>% as.numeric,
            j = gsub(ptrn, "\\5", a) %>% as.numeric,
            k = gsub(ptrn, "\\6", a) %>% as.numeric,
            model = gsub(ptrn, "\\3", a),
            pert = gsub(ptrn, "\\2", a) %>% as.numeric,
            n = gsub(ptrn, "\\1", a) %>% as.numeric) %>% 
  group_by(j, k, model, pert, n) %>% 
  summarize(n = n(), n.short = 10 - n) %>% 
  filter(n.short > 0) %>% 
  as.data.frame
