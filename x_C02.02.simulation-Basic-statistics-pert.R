    ...
    for (k in 1:length.parameter) {
      ...
      if (row.index > 0) {
        ...
        result.stat[row.index,] = 
          c(i=i, j=j, k=k, rds$pval %>% apply(1, function(x) {mean(x<=sig, na.rm=TRUE)}) )
        result.stat.na[row.index,] = 
          c(i=i, j=j, k=k, rds$pval %>% apply(1, function(x) {mean(ifelse(is.na(x), 1, x) <= sig, na.rm=TRUE)}) )
...
result.stat.na %>% 
  ...
  dplyr::select(-c(4,5,6,8,9,10,12,13,14,15)) -> result.stat.na

##################### !!! plot for main article !!! ##############################

test = c("LB", "LN", "MAST", "KW",  "KW-II")
