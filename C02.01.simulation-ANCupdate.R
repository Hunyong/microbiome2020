...
for (j in 1:5) {
  ...
  for (i in rng) {
    ...
    result <- tester.set.HD.batch(data, n.gene=n.gene, suppressWarnWagner = TRUE, # if not suppressed, the slurm-out file size explodes.
                                  ...
                                  MGS.skip = T, ANCOM.skip = F, skip.cumulative = TRUE) # if there are batch effects skip WRS and MGS.
    ...
    final <- list(stat = stat.comb, cdf = result$pval.cdf, setting = setting.summary)
    
    # Filling in the existing results
    existing <- readRDS(read_file.stat)
    existing.methods = c("LB.nonz", "LB.zero", "LB.glob", "LB.min", "LN", "MAST.nonz", "MAST.zero", "MAST.glob", "MAST.min", 
                         "KW", "Wg.nonz", "Wg.zero", "Wg.glob", "Wg.min", "DS2", "DS2ZI", "WRS", "MGS")
    final$stat[, existing.methods] = existing$stat[, existing.methods]
    final$cdf[existing.methods, ] = existing$cdf[existing.methods, ]
    
    saveRDS(final, save_file.stat)
