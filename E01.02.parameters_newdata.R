...
for (type in c("genebact", "bact", "gene")) {
  ...
  DataMeta <-
    DataMeta %>% 
    mutate(group_batch =  ifelse(site_name %in% c("Cedars-Sinai", "MGH"), 1, 2), # dichotomize sites into either related to adult cohort or a pediatric one
           # three paediatric sub-cohorts: Cincinnati Children’s Hospital, Massachusetts General Hospital (MGH) Pediatrics, and Emory University Hospital; and two adult cohorts: MGH and Cedars- Sinai Medical Center
           # group_batch =  ifelse(grepl("MGH", site_name), 1, 2), # dichotomize sites into either related to MGH (MGH and MGH pediatrics)
