...
metricsplot <- function(model, size,  width = 12,  height = 8, metrics.name = "type1error", delta.base = TRUE, 
                      fn = paste0("figure/", model, "_", metrics.name, "_size", size,  if (!delta.base) "_effectSize(no_batch)",  
                      ...
                      res.tmp = TRUE, include.null = FALSE, stop.if.absent = TRUE)
{
  ...
  if (delta.base) {
    ...
    disease.labels = c(TeX(r"(D2 $\mu_D$>$\mu_{H}$)"),  
  ...
  res %>%
    ...
    {if (delta.base) 
      scale_fill_manual(name = TeX(r"(Batch effects $\kappa_\mu, \kappa_\theta, \kappa_{\pi}$)"),
...
# metricsplot(model = "ziln", size = 80, include.null = TRUE, stop.if.absent = FALSE)
# metricsplot(model = "ziln", size = 400, delta.base = FALSE, stop.if.absent = FALSE) # disease effect sensitivity analysis
# metricsplot(model = "ziln", size = 80, delta.base = FALSE, stop.if.absent = FALSE) # disease effect sensitivity analysis

# metricsplot(model = "zig", size = 400, stop.if.absent = FALSE)
# metricsplot(model = "zig", size = 80, stop.if.absent = FALSE)

# metricsplot(model = "zinb", size = 400, stop.if.absent = FALSE)
# metricsplot(model = "zinb", size = 80, stop.if.absent = FALSE)
