...
for l in 3; do # ZILN
  for j in 1 3 5; do 
    for k in 7 9 10 12 25 27 28 30 43 45 46 48; do 
      if [[ "$j" = 1 ]]; then
      sbatch --output=./Report/slurm-%j.out --time=16:00:00 --mem 10000 bash/2run-code-metrics.sh 0 $j $k $l 5 80 0.1;   # i ranges 1:16
      sbatch --output=./Report/slurm-%j.out --time=64:00:00 --mem 10000  bash/2run-code-metrics.sh 0 $j $k $l 5 400 0.1; # i ranges 1:16 
      else 
      sbatch --output=./Report/slurm-%j.out --time=10:00:00 --mem 10000 bash/2run-code-metrics.sh 0 $j $k $l 5 80 0.1;   # i ranges 1:10
      sbatch --output=./Report/slurm-%j.out --time=40:00:00 --mem 10000  bash/2run-code-metrics.sh 0 $j $k $l 5 400 0.1; # i ranges 1:10
      fi;
    done; 
  done; 
done;
