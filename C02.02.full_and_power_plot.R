...
powercurve <- function(model, width = 12, height = 9,
  ...
  for (size in n.rng) {
    for (i in i.rng) {
      for (j in j.rng) {
        ...
        for (k in k.rng) {
          cat(k, " ")
          fn.tmp <- paste0("output-no-replica/stat-n", size, "-pert0.5-signal0.1-", model, "-", i, ".", j, ".", k, ".rds")
          if (file.exists(fn.tmp)) {
            result <- readRDS(paste0("output-no-replica/stat-n", size, "-pert0.5-signal0.1-", model, "-", i, ".", j, ".", k, ".rds"))
  ...
  res <- res %>%
    gather(
      key = "method", value = "rejection.rate",
      `LB`, `LN`, `MAST`, `KW`, `KW-II`, `DS2`, `DS2ZI`, `MGS`, `ANCOM`
    )
  res$method_f <- factor(res$method,
    levels = c("LN", "LB", "MAST", "DS2", "DS2ZI", "MGS", "ANCOM", "KW", "KW-II"),
